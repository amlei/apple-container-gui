...
/// SwiftUI-rendered plain style: on macOS a `.plain` button can be backed by
/// `NSButton`, whose hit area only covers the label text. A custom style keeps
/// the button in SwiftUI so `.contentShape` makes the whole area clickable.
struct SQPlainButtonStyle: ButtonStyle {
